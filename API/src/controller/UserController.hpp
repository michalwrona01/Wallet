...
public:
  ...
  ENDPOINT_INFO(getUserByEmail){
    info->summary = "Get one User by email";

    info->addResponse<Object<UserDto>>(Status::CODE_200, "application/json");
    info->addResponse<Object<StatusDto>>(Status::CODE_404, "application/json");
    info->addResponse<Object<StatusDto>>(Status::CODE_500, "application/json");

    info->pathParams["email"].description = "User emial";
  }
  ENDPOINT("GET", "users/email/{email}", getUserByEmail,
            PATH(String, email))
    {
        return createDtoResponse(Status::CODE_200, m_userService.getUserByEmail(email));
    }
